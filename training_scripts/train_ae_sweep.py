...
def wandb_sweep(project_name = 'ae_reshuffle_PILoss_sweep', count=200):
        ...
        sweep_config = {
            ...
            'parameters': { 'learning_rate' : {"values": [0.001, 0.0005, 0.0001]},
                            ...
                            # 'latent_dim'    : {"values": [128, 256, 1024]},
                            'acti_funct'    : {'values': ['SELU', 'ReLU', 'LeakyReLU', 'Tanh']},
                            }
                        }
        
        sweep_id = wandb.sweep(sweep=sweep_config, project=project_name)
        wandb.agent(sweep_id, sweep_ae, count=count)
        wandb.finish()
...
def get_activation_function(name):
    ...
    if name == 'SELU':
        ...
        raise TypeError('Invalid activation function name')
...
def sweep_ae():
    ...
    with wandb.init() as run:
        ...
        acti_funct      = get_activation_function(config.acti_funct)
        ...
        param_dict  = {'n_layers'           : 4,
                       'init_channel'       : 32,
                       'latent_dim'         : 1024, 
                       'activation_function': acti_funct
                       }
